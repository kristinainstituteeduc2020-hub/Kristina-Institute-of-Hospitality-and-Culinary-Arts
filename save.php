...
// TESDA category (admin-driven)
$tesda_category = null;
if(in_array($type, ['tesda_assessment','tesda_training'], true)){
  $tesda_category = trim($_POST['tesda_category'] ?? '');
}


// TESDA Assessment/Training: store content exactly as entered (NO JSON generation).
// News/Updates and TESDA share the same save behavior: insert raw textarea content into `posts.content`.
if(in_array($type, ['tesda_assessment','tesda_training'], true)){
  // No modifications to $content; keep plain text exactly as received.
} 
...
if($title === ''){
  header('Location: dashboard.php?type=' . urlencode($type) . '&err=' . urlencode('Title is required.'));
  exit;
}

// Content is optional for TESDA qualification posts so users can upload media
// and rely on TESDA title-based program mapping.
$needsContent = !in_array($type, ['tesda_assessment','tesda_training'], true);
if($needsContent && $content === ''){
  header('Location: dashboard.php?type=' . urlencode($type) . '&err=' . urlencode('Content is required.'));
  exit;
}
...
$tesdaCategory = null;
if(in_array($type, ['tesda_assessment','tesda_training'], true)){
  $tesdaCategory = trim($_POST['tesda_category'] ?? '');
  if($tesdaCategory === '') $tesdaCategory = null;
}

$stmt = $pdo->prepare('INSERT INTO posts (type, title, content, author_name, image_path, attachment_path, video_path, tesda_category, created_at) VALUES (:type,:title,:content,:author,:img,:att,:vid,:cat,NOW())');
$stmt->execute([
  ':type'=>$type,
  ':title'=>$title,
  ':content'=>$content,
