...
$tesdaCategoryFilter = trim($_GET['tesda_category'] ?? '');
if(!in_array($type, ['tesda_assessment','tesda_training'], true)){
  $tesdaCategoryFilter = '';
}
...
$items = [];
$sql = "SELECT id, type, title, content, created_at, tesda_category
        FROM posts
        WHERE type = :type
          AND (:t = '' OR title LIKE CONCAT('%', :t, '%'))";

$params = [':type'=>$type, ':t'=>$filter_title];

if(($type==='tesda_assessment' || $type==='tesda_training') && $tesdaCategoryFilter !== ''){
  $sql .= " AND tesda_category = :cat";
  $params[':cat'] = $tesdaCategoryFilter;
}
...
<body>
  ...
  <div class="admin-wrap">
    ...
    <div class="grid2">
        ...
        <div class="muted">
          <?php echo ($type==='tesda_assessment' || $type==='tesda_training') ? 'Upload image/video/attachment optional.' : 'Upload an image (optional), attachment (optional), and video (optional).'; ?>
        </div>
        ...
        <form method="post" action="save.php" enctype="multipart/form-data">
          ...
          <?php if($type==='tesda_assessment' || $type==='tesda_training'): ?>
            <div class="small" style="margin:10px 0 12px; color:var(--muted);">
              For TESDA posts: choose a category, then upload media (image/video/attachment). Title is required.
            </div>

            <?php $selectedTesdaCategory = $_GET['tesda_category'] ?? 'UTPRAS'; ?>
            <label>TESDA Category</label>
            <select name="tesda_category" required>
              <option value="UTPRAS" <?php echo ($selectedTesdaCategory==='UTPRAS') ? 'selected' : ''; ?>>UTPRAS</option>
              <option value="Assessment Certification & Accreditation" <?php echo ($selectedTesdaCategory==='Assessment Certification & Accreditation') ? 'selected' : ''; ?>>Assessment Certification & Accreditation</option>
              <option value="Community-Based Training Qualifications" <?php echo ($selectedTesdaCategory==='Community-Based Training Qualifications') ? 'selected' : ''; ?>>Community-Based Training Qualifications</option>
            </select>
          <?php endif; ?>
          ...
          <?php if($type==='tesda_assessment' || $type==='tesda_training'): ?>
            <textarea name="content" placeholder="Optional" style="min-height:120px;"></textarea>
          <?php else: ?>
